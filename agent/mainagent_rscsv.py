...
# 而 streamlit run / python agent/mainagent_rscsv.py 都只会把 agent 目录放进 sys.path
...
from tools.agent_tools import rag_summarize,get_weather,get_user_location, get_user_id, rag_rscsv_rscsv, to_openai_tools, rag_rscsv_service_rscsv
from tools.middleware import monitor_tool,log_before_model,report_prompt_switch,calculate_hit_rate
from utils.logger_handler import logger

# 与 mainagent.py 保持结构一致，唯一差异：检索工具（rag_rscsv_rscsv → 切片库全量精确检索）
# 用于 main_eval MODEL_KEY="agent-text-doubao-seed_rscsv" 的受控对比（除检索策略外其余全同）
class MainAgent:
    def __init__(self, model=None, tools=None):
        """Args:
            model: 聊天模型实例（None 时用默认 doubao_seed_20_mini_model）
            tools: 工具列表（None 时用默认 [rag_rscsv_rscsv]）
        """
        self.tools = tools if tools is not None else [rag_rscsv_rscsv]
        self.model = model
    ...
    def get_last_trace(self):
        """获取最后一次 RAG 检索的过程记录（含耗时拆分，供统计/UI 展示）"""
...
if __name__=="__main__":
    ...
    # image_path = "C:\\dataset\\SAR-TEXT\\SAR-TEXT-data\\Image\\whu-sar-opt\\NH49E010014_2_3.tif"
    image_path = "C:\\dataset\\SAR-TEXT\\SAR-TEXT-data\\Image\\OSdataset_256_train\\sar2839.png"
    # image_path = ""
    ...
    query = "Could the parking lot be part of a recreational area, like a park or trailhead?"
    image_file = None
