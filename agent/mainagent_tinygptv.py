...
# 路径注入必须早于任何项目内 import（与 mainagent_internVL.py 一致）
...
class MainAgent(_InternVLMainAgent):
    """两步式 agent：doubao 收集 RAG（文本）→ tinygptv 多模态最终回答"""

    def __init__(self, model=None, vision_model=None, tools=None):
        super().__init__(
            model=model,
            vision_model=vision_model if vision_model is not None else tinygptv_model,
            tools=tools,
        )
