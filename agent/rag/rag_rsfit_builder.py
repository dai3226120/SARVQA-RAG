...
# ================================================================
# RAG 检索配置
RETRIEVAL_K = chroma_conf["k"]                          # 检索返回的切片数量
...
class SARSemanticCacheSystem:
    ...
    def __init__(self, force_full_reload: bool = False):
        # ========================================
    ...
    def call_vlm_agent(self, image_path, question, ground_truth=None):
        ...
        """
        # 懒加载：首次调用时初始化Agent（避免循环导入）
        if not self._vlm_agent_initialized:
            self._init_vlm_agent()
            self._vlm_agent_initialized = True

        if self.vlm_agent is None:
            logger.error("[VLM Agent] Agent未初始化，使用默认多模态模型")
        ...
        try:
            ...
            RscsvServiceRscsv.reset_last_retrieved_slice_ids()

            with open(image_path, 'rb') as image_file:
                response_chunks = []
                for chunk in self.vlm_agent.execute_stream(query=question, image_file=image_file):
                    response_chunks.append(chunk)
                response_text = "".join(response_chunks).strip()

            # 从agent实例获取实际使用的切片ID（优先从agent属性获取，兼容子线程工具调用的场景）
            slice_ids = getattr(self.vlm_agent, 'last_slice_ids', [])
            if not slice_ids:
                # fallback：从线程局部存储获取
                slice_ids = RscsvServiceRscsv.get_last_retrieved_slice_ids()
            logger.info(f"[VLM Agent] 本次调用实际使用的切片ID: {slice_ids}")
    ...
    def _check_and_reload_vector_db(self):
        """检查 CSV 文件的 MD5 状态，若发生改变或未记录，则全量清空并重载"""
        try:
            # 调用 utils/file_handler.py 的标准 MD5 获取函数
            current_md5 = get_file_md5_hex(self.log_path)
        except Exception as e:
            logger.error("计算日志文件 MD5 失败: %s", e)
            return

        if not current_md5:
            logger.error("生成的日志文件 MD5 为空，跳过重载检查。")
            return

        # 如果当前文件的 MD5 没在记录中找到，代表文件已被更新或尚未被该模块录入
        if not self._is_md5_registered(current_md5):
            logger.warning("[MD5未匹配] 检测到日志文件已变更或尚未加载至向量数据库。开始触发全量重载...")
            
            # 1. 重新全量同步到 Chroma
            self._reload_logs_to_vector_db()
            
            # 2. 将此版新的 MD5 追加标记为已处理，防止下一次重复载入
            self._append_md5_record(current_md5)
        else:
            logger.info("[MD5校验通过] 向量数据库中的数据与当前日志文件版本完全一致。")
    ...
    def _reload_logs_to_vector_db(self):
        ...
        try:
            # 关键修复2：彻底清空向量库（先获取所有ID再删除）
            existing_items = self.logs_collection.get()
            existing_ids = existing_items.get("ids", [])
            if existing_ids:
                self.logs_collection.delete(ids=existing_ids)
                logger.info("已清空旧的日志向量集合，共释放 %d 条记录。", len(existing_ids))
            
            # 新版 langchain-chroma 已移除 persist()，自动持久化
        except Exception as e:
            logger.error("清空旧日志向量集合失败: %s", e)
            # 尝试直接删除整个集合（极端情况）
            try:
                self.logs_collection.delete_collection()
                self.logs_collection = Chroma(
                    collection_name=Config.collection_name,
                    embedding_function=self.embeddings,
                    persist_directory=self.persist_directory,
                ...
                logger.info("已重建空的日志向量集合")
            except Exception as e2:
                logger.error("重建向量集合失败: %s", e2)
                return
        ...
        if texts:
            ...
            for i in range(0, len(texts), batch_size):
                ...
                try:
                    ...
                    logger.info(f"已添加第 {i//batch_size + 1} 批数据，共 {len(batch_texts)} 条")
                except Exception as e:
                    logger.error(f"添加第 {i//batch_size + 1} 批数据失败: {e}")
                    continue
...
class RealRAGEngine:
    ...
    def __init__(self):
        ...
        self.retriever = self.builder.get_retriever(self.slice_collection)

    def search(self, query, k=Config.retrieval_k):
        """从向量库中检索相关切片
...
def run_real_test(force_full_reload: bool = False, rejudge: bool = False):
    """
    运行真实数据的测试

    Args:
        force_full_reload: 是否强制全量更新向量库（忽略MD5校验）
        rejudge: 是否对现有日志进行一次性LLM重判（用于数据迁移）
    """

    logger.info('=== 开始 SARSemanticCacheSystem 真实数据测试 ===')

    # 创建系统实例（支持强制全量更新）
    system = SARSemanticCacheSystem(force_full_reload=force_full_reload)
    ...
    # 一次性重判模式：跳过评估，直接对现有日志进行LLM判断
    if rejudge:
        logger.info('--- [一次性重判] 对现有日志进行LLM判断 ---')
        try:
            system.rejudge_existing_logs(test_df)
        except Exception as e:
            logger.exception('[一次性重判] 出错: %s', e)
        logger.info('=== 一次性重判完成 ===')
        return
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='SARSemanticCacheSystem 测试')
    parser.add_argument('--force-full-reload', action='store_true',
                       help='强制全量更新向量库（忽略MD5校验，清空后重新加载）')
    parser.add_argument('--rejudge', action='store_true',
                       help='一次性重判：对现有日志进行LLM判断过滤（用于数据迁移，完成后自动恢复）')
    args = parser.parse_args()

    run_real_test(force_full_reload=args.force_full_reload, rejudge=args.rejudge)