...
def load_system_prompts():
    try:
        ...
        logger.error(f"[load_system_prompts]在yaml配置项中没有system_prompt_path配置项")
        raise e
    try:
...
def load_report_prompts():
    ...
    try:
        ...
        logger.error(f"[load_report_prompts]解析报告生成系统提示词出错，{str(e)}")
        raise e

def load_agent_prompts():
    try:
        agent_prompt_path = get_abs_path(prompts_conf["agent_prompt_path"])
    except KeyError as e:
        logger.error(f"[load_agent_prompts]在yaml配置项中没有agent_prompt_path配置项")
        raise e
    try:
