...
from agent_client import internvl_agent_client
...
def process_single_row(index, row, image_base_path):
    ...
    try:
        ...
        # 调用Agent
        predicted_answer = internvl_agent_client.call(image_path_front, question_text)
        predicted_answer = clean_special_chars(predicted_answer)

        # 获取检索耗时
        retrieval_latency = internvl_agent_client.get_last_retrieval_latency()
...
def main():
    print("=" * 80)
    print("[START] SAR-VQA 数据处理脚本（InternVL + Agent）")
    print("=" * 80)

    # 配置参数
    file_tag = get_file_tag(ModelType.AGENT_INTERNVL)
    dataset_tag = "val"
    ...
    safe_print(f"   - 模型类型: {ModelType.AGENT_INTERNVL}")
    ...
    # 输出Agent调用统计
    internvl_agent_client.print_stats()

    # 输出指标汇总
    print("\n[STATS] 检索与信息指标汇总:")
    for line in retrieval_metrics.format_summary(internvl_agent_client):
        print(f"  {line}")
