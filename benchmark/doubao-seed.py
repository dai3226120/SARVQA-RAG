...
from config import ModelType, get_file_tag, get_csv_input_path, get_result_dir, get_base_filename, doubao_config
from utils import check_csv_structure, safe_print
from api_client import call_doubao_api, doubao_client
...
def main():
    print("=" * 80)
    print("[START] SAR-VQA 数据处理脚本（豆包Seed）")
    print("=" * 80)

    # 配置参数
    file_tag = get_file_tag(ModelType.DOUBAO)
    dataset_tag = "val"
    ...
    safe_print(f"   - 模型类型: {ModelType.DOUBAO}")
    ...
    safe_print(f"   - API端点: {doubao_config.API_ENDPOINT}")
    safe_print(f"   - 模型名称: {doubao_config.MODEL_NAME}")
    safe_print("-" * 80)
    ...
    process_func = create_process_row_func(call_doubao_api, include_metrics=True)
