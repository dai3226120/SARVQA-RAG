...
df_stats = pd.read_csv('./utils/dataset_analysis_result/问题-出现的数据集.csv')
# 获取出现总次数大于50的问题集合
questions_to_remove = set(df_stats[df_stats['出现总次数'] > 50]['问题'])

# 2. 读取主数据集
df_main = pd.read_csv('./SAR-VQA_ALL-246980_utf8.csv')
...
# 4. 保存结果到新文件
df_filtered.to_csv('./SAR-VQA_ALL_filtered_question-re50+.csv', index=False, encoding='utf-8')
